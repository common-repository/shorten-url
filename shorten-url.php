 ...
 * @license   GPL-3.0+
 * @link      https://wordpress.org/plugins/shorten-url/
...
if ( ! defined( 'KC_SU_PLUGIN_DIR' ) ) {
	define( 'KC_SU_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
}
...
class shorturl extends pluginSedLex {
	...
	*/
	...
	public function get_accessible_commands() {
		return array(
			...
			'valid_link_external',
			'cancel_link',
			'cancel_link_external',
			'delete_link_external',
		);
	}
    ...
    public function handle_request() {
	    ...
	    check_ajax_referer( KC_SU_AJAX_SECURITY, 'security' );

        $cmd = $params['cmd'];

	    if ( in_array( $cmd, $this->get_accessible_commands() ) && is_callable( array( $this, $cmd ) ) ) {
		    $this->$cmd( $params );
	    }
    }
	...
	function configuration_page() {
		...
		if (isset($_POST['import'])) {
			...
			ob_start() ; 
				...
				if (isset($_POST['add'])) {
					$url_ext = sanitize_text_field(str_replace("'", "", $_POST['url_externe'])) ;
					$comment = sanitize_text_field(str_replace("'", "", $_POST['comment'])) ;
	...
	*/
	function reset_link() {
        ...
        $q = $wpdb->prepare("DELETE FROM {$table_name} WHERE id_post=%d", $idLink);
		$wpdb->query( $q ) ;
		// Create a new entry
...
if ( ! defined( 'KC_SU_MIN_PHP_VER' ) ) {
	define( 'KC_SU_MIN_PHP_VER', '5.6' );
}

if ( ! defined( 'KC_SU_MAX_PHP_VER' ) ) {
	define( 'KC_SU_MAX_PHP_VER', '7.4.33' );
}

if ( ! defined( 'KC_SU_AJAX_SECURITY' ) ) {
	define( 'KC_SU_AJAX_SECURITY', 'shorten_url_ajax_security' );
}


if ( ! function_exists( 'kc_su_fail_php_version_notice' ) ) {
	/**
	 ...
	 *
	 */
	function kc_su_fail_php_version_notice() {
		/* translators: %s: PHP version */
		$message      = sprintf( esc_html__( 'Shorten URL requires PHP version between %s & %s, plugin is currently NOT RUNNING.', 'shorten-url' ), KC_SU_MIN_PHP_VER,  KC_SU_MAX_PHP_VER);
		$html_message = sprintf( '<div class="error">%s</div>', wpautop( $message ) );
		echo wp_kses_post( $html_message );
	}
}

if ( ! version_compare( PHP_VERSION, KC_SU_MIN_PHP_VER, '>=' ) || ! version_compare( PHP_VERSION, KC_SU_MAX_PHP_VER, '<=' )) {
	add_action( 'admin_notices', 'kc_su_fail_php_version_notice' );

	return;
}

if ( ! function_exists( 'kc_su_migrate_to_url_shortify' ) ) {
	/**
	 * Shorten URL admin notice for minimum & maximum PHP version.
	 *
	 * Warning when the site doesn't have the minimum required PHP version.
	 ...
	 *
	 */
	function kc_su_migrate_to_url_shortify() {
		/* translators: %s: PHP version */
		$message      = sprintf( __('We are retiring Shorten URL plugin and will continue to develop <a href="%s" target="_blank">URL Shortify</a>. We will provide help to migrate to URL Shortify. <a href="mailto:hello@kaizencoders.com">Contact us</a> for migration. Migrate to URL Shortify and use coupon code <b class="kc-su-migrate-code">MIGRATE</b>  to get flat <b class="kc-su-migrate-code">%s</b> discount on new purchase and renewal on all <a href="https://kaizencoders.com/url-shortify/" target="_blank">URL Shortify PRO</a>  plans.','shorten-url'), 'https://wordpress.org/plugins/url-shortify/', '20%');

		$html_message = sprintf( '<div class="notice notice-info kc-su-migration-notice" style="">%s</div>', wpautop( $message ) );

		echo wp_kses_post( $html_message );
	}
}

add_action( 'admin_notices', 'kc_su_migrate_to_url_shortify' );
